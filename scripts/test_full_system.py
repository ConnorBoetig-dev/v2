...
def test_full_system():
    ...
    for device in classified:
        print(
            f"     - {device['ip']}: {device['type']} (confidence: {device.get('classification_confidence', 'N/A')})"
        )
    ...
    test_config = {"version": "v2c", "community": "public", "timeout": 2, "retries": 1}
    ...
    annotation = DeviceAnnotation(
        ip="192.168.1.1", critical=True, notes="Main router - critical infrastructure"
    )
