...
async def test_complete_flow():
    ...
    scan_data = {
        "scan_metadata": {
            ...
            "scan_type": "deeper",
            "scan_parameters": {"snmp_enabled": False, "vulnerability_scan": False},
        },
    ...
    print("  " + "-" * 100)
    print(
        f"  {'IP Address':<15} {'Hostname':<20} {'MAC Address':<20} {'Vendor':<30} {'OS':<10} {'Services'}"
    )
    print("  " + "-" * 100)
    ...
    for device in mock_devices:
        ...
        print(
            f"  {device['ip']:<15} {device.get('hostname', '-'):<20} {device.get('mac', '-'):<20} "
