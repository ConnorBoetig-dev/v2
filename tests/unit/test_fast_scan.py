                ...
                with patch.object(scanner, "_check_scanner_available", return_value=True):
                    ...
                    with patch.object(scanner, "_estimate_total_hosts", return_value=65534):
                        with patch.object(scanner, "_ensure_sudo_access", return_value=True):
                            with patch.object(
                                scanner, "_create_temp_file", return_value="/tmp/test.json"
                            ):
    ...
    def test_fast_scan_interface_detection(self, scanner):
        ...
        with patch("subprocess.Popen") as mock_popen:
            with patch.object(scanner, "_parse_masscan_output", return_value=[]):
                with patch.object(scanner, "_check_scanner_available", return_value=True):
                    with patch.object(
                        scanner, "_get_best_interface_for_target", return_value="eth0"
                    ):
                        with patch.object(
                            scanner, "_get_source_ip_for_interface", return_value="192.168.1.100"
                        ):
                            mock_proc = Mock()
                            mock_proc.poll.side_effect = [None, 0]
                            ...
                            with patch.object(scanner, "_estimate_total_hosts", return_value=65534):
                                with patch.object(
                                    scanner, "_ensure_sudo_access", return_value=True
                                ):
                                    with patch.object(
                                        scanner, "_create_temp_file", return_value="/tmp/test.json"
                                    ):
