...
class NetworkDataFactory:
    ...
    def __init__(self, seed: int = 42):
        ...
        self.port_profiles = {
            'router': {
                'common': [22, 23, 80, 161, 443],
                'optional': [8080, 8443, 179, 520]
            },
            'switch': {
                'common': [22, 23, 80, 161],
                'optional': [443, 8080]
            ...
            'web_server': {
                'common': [22, 80, 443],
                'optional': [8080, 8443, 9080, 9443]
            },
            'database': {
                'common': [22, 3306, 5432],
                'optional': [1433, 5984, 6379, 27017]
            },
            'mail_server': {
                'common': [22, 25, 110, 143, 993, 995],
                'optional': [587, 465]
            },
            'file_server': {
                'common': [22, 139, 445],
            ...
            'workstation': {
                'common': [135, 139, 445],
                'optional': [22, 3389, 5900]
            },
            'printer': {
                'common': [80, 443, 631, 9100],
                'optional': [515, 721]
            },
            'security_camera': {
                'common': [80, 554],
                'optional': [443, 8080, 8554]
            },
            'iot_device': {
                'common': [80, 443],
                'optional': [1883, 8883, 5683]
            },
            'access_point': {
                'common': [22, 80, 443],
                'optional': [161, 8080]
        ...
        self.port_services = {
            ...
            515: 'printer', 554: 'rtsp', 587: 'submission',
            631: 'ipp', 993: 'imaps', 995: 'pop3s',
            1433: 'mssql', 1883: 'mqtt', 2049: 'nfs',
    ...
    def generate_device(self, device_type: str = None, subnet: str = "192.168.1") -> Dict[str, Any]:
        ...
        vendor = random.choice(suitable_vendors) if suitable_vendors else random.choice(list(self.vendors.keys()))
        ...
        optional_ports = random.sample(
            port_profile['optional'], 
            random.randint(0, min(3, len(port_profile['optional'])))
        )
        
        device['open_ports'] = sorted(common_ports + optional_ports)
        device['services'] = [self.port_services.get(port, f'unknown-{port}') 
    ...
    def generate_network(self, 
                        ...
                        device_distribution: Dict[str, float] = None) -> List[Dict[str, Any]]:
    ...
    def generate_scan_timeline(self, 
                             ...
                             num_scans: int = 5,
                             days_apart: int = 1) -> List[Dict[str, Any]]:
    ...
    def _generate_os(self, device_type: str, vendor: str) -> str:
        ...
        os_map = {
            ...
            ('web_server', 'VMware'): random.choice(['Ubuntu 20.04', 'CentOS 7', 'Windows Server 2019']),
            ('database', 'VMware'): random.choice(['Ubuntu 20.04', 'CentOS 8', 'Windows Server 2019']),
        ...
        if device_type in ['workstation']:
            ...
            return random.choice(['Ubuntu 20.04', 'CentOS 8', 'Windows Server 2019', 'Windows Server 2022'])
        else:
            return 'Linux'
        ...
        if random.random() < 0.2 and changed_network:
            ...
            if random.random() < 0.5:  # Add service
                ...
                if new_port not in device['open_ports']:
                    ...
                    device['services'].append(self.port_services.get(new_port, f'unknown-{new_port}'))
            else:  # Remove service
                if len(device['open_ports']) > 1:
                    removed_port = random.choice(device['open_ports'])
                    device['open_ports'].remove(removed_port)
                    service_to_remove = self.port_services.get(removed_port, f'unknown-{removed_port}')
