...
def scan(
    targets: Optional[List[str]] = typer.Argument(
        None,
        help="Target networks or hosts to scan"
    ),
    scan_type: Optional[str] = typer.Option(
        None,
        "--type", "-t",
        help="Scan type: discovery, inventory, deep, arp, fast"
    ),
    ...
    format: str = typer.Option(
        "json",
        "--format", "-f",
        help="Output format: json, csv, html, pdf, excel"
    ),
    no_enrich: bool = typer.Option(
        False,
        "--no-enrich",
        help="Skip enrichment phase (SNMP, DNS)"
    ),
    ...
    with Progress(
        ...
        try:
            result = asyncio.run(
                _context.scan_orchestrator.execute_scan(
                    options,
                    progress_callback
                )
            )
...
def list_devices(
    search: Optional[str] = typer.Option(
        None,
        "--search", "-s",
        help="Search devices by IP, hostname, or type"
    ),
    ...
    critical_only: bool = typer.Option(
        False,
        "--critical",
        help="Show only critical devices"
    )
):
    ...
    devices = asyncio.run(
        _context.scan_orchestrator.repository.get_all_devices()
    )
    ...
    if device_type:
        devices = [
            d for d in devices
            if d.device_type.value == device_type.lower()
        ]
...
def annotate(
    ...
    notes: Optional[str] = typer.Option(
        None,
        "--notes", "-n",
        help="Add notes to device"
    ),
    tags: Optional[List[str]] = typer.Option(
        None,
        "--tag", "-t",
        help="Add tags to device"
    ),
    critical: Optional[bool] = typer.Option(
        None,
        "--critical",
        help="Mark device as critical"
    ),
    ...
    # Get device
    device = asyncio.run(
        _context.scan_orchestrator.repository.get_device_by_ip(ip_address)
    )
    ...
    # Save
    asyncio.run(
        _context.annotation_service.update_annotations(device)
    )
...
def compare(
    ...
    output: Optional[Path] = typer.Option(
        None,
        "--output", "-o",
        help="Export comparison results"
    )
):
    ...
    try:
        result1 = asyncio.run(
            _context.scan_orchestrator.repository.load_scan_file(scan1)
        )
        result2 = asyncio.run(
            _context.scan_orchestrator.repository.load_scan_file(scan2)
        )
    except Exception as e:
...
def export(
    ...
    format: str = typer.Option(
        "json",
        "--format", "-f",
        help="Export format: json, csv, html, pdf, excel"
    )
):
    ...
    try:
        result = asyncio.run(
            _context.scan_orchestrator.repository.load_scan_file(scan_file)
        )
    except Exception as e:
