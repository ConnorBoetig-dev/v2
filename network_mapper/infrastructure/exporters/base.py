    ...
    def validate_output_path(self, output_path: Path) -> None:
        ...
        if self.config.get('strict_extension', True):
            ...
            if output_path.suffix != expected_ext:
                raise ExportError(
                    f"Invalid file extension. Expected {expected_ext}, "
    ...
    def prepare_export_data(self, scan_result: ScanResult) -> Dict[str, Any]:
        ...
        return {
            'metadata': {
                ...
                'duration': (
                    scan_result.metadata.end_time - 
                    scan_result.metadata.start_time
                ).total_seconds(),
            ...
            'devices': [
                device.to_dict() for device in scan_result.devices
            ],
