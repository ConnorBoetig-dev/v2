...
def validate_scan_arguments(
    ...
    forbidden_args = [
        '-oA', '-oN', '-oG', '-oS',  # Output format conflicts
        '-iL', '-iR',  # Input conflicts
...
def merge_scan_options(
    ...
    if custom_ports:
        ...
        config['args'] = [
            ...
            if not (arg == '-p' or arg.startswith('--top-ports'))
        ]
        config['args'].extend(['-p', custom_ports])
    
    # Add custom arguments
    if custom_args:
        validated_args = validate_scan_arguments(
            ScanType(profile.name.lower()), 
            custom_args
        )
