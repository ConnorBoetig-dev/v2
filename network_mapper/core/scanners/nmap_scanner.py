...
class NmapScanner(Scanner):
    ...
    def is_available(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["nmap", "--version"], capture_output=True, text=True, check=True
            )
    ...
    def _execute_scan(self, cmd: List[str], options: ScanOptions) -> str:
        ...
        if self._process.returncode != 0:
            ...
            raise subprocess.CalledProcessError(
                self._process.returncode, cmd, stderr=stderr.encode()
            )

        return "".join(output_lines)

    def _parse_progress(
        self, line: str, discovered_hosts: int, current_host: Optional[str]
    ...
    def _requires_sudo(self, options: ScanOptions) -> bool:
        ...
        return options.scan_type in [ScanType.INVENTORY, ScanType.DEEP, ScanType.ARP]
