...
class ScanOrchestrator:
        ...
        try:
            ...
            if self.enrichers and not self._is_cancelled:
                await self._update_progress(
                    progress_callback,
                    phase="enrichment",
                    message="Enriching device information..."
                )
            ...
            if not self._is_cancelled:
                await self._update_progress(
                    progress_callback,
                    phase="tracking",
                    message="Tracking network changes..."
                )
    ...
    async def _track_changes(
        ...
        try:
            ...
            previous_scan = await self.repository.get_latest_scan(
                targets=options.targets
            )
            
            if previous_scan:
                return self.change_tracker.compare_scans(
                    previous_scan.devices,
                    devices
                )
        except Exception as e:
    ...
    def _calculate_risk_score(self, device: Device) -> float:
        ...
        total_score = sum(
            severity_weights.get(v.severity.lower(), 0)
