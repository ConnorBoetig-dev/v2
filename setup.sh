...
# Exit immediately if a command exits with a non-zero status.#!/bin/bash
#
# NetworkMapper v2 - Automated Local Setup Script
#
...
check_os() {
    ...
    if [[ "$OS_TYPE" == "linux" ]]; then
        ...
        log_error "Unsupported OS: $OS_TYPE. Please use Linux, macOS, or WSL on Windows."
        exit 1
    fi
}
...
check_python() {
    ...
    log_info "Python ${PY_VERSION_MAJOR}.${PY_VERSION_MINOR} found."
}

install_system_deps() {
    log_step "Checking system-wide dependencies..."
    deps=("nmap" "masscan" "arp-scan")
    missing_deps=()
    for dep in "${deps[@]}"; do
        if command_exists "$dep"; then log_info "$dep is installed."; else log_warn "$dep is NOT installed."; missing_deps+=("$dep"); fi
    ...
    if [ ${#missing_deps[@]} -ne 0 ]; then
        ...
        read -p "Attempt to install them now? (y/N) " -n 1 -r; echo
        if [[ ! $REPLY =~ ^[Yy]$ ]]; then log_error "Aborting. Please install missing dependencies manually."; exit 1; fi
        if [[ "$OS_TYPE" == "linux" ]]; then
            log_step "Attempting to install with apt..."; sudo apt-get update; sudo apt-get install -y "${missing_deps[@]}"
        elif [[ "$OS_TYPE" == "darwin" ]]; then
            if ! command_exists brew; then log_error "Homebrew not found. Please install it: https://brew.sh/"; exit 1; fi
...
create_directories() {
    log_step "Creating necessary output directories..."
    mkdir -p output/{scans,reports,exports,logs,cache,config,changes,annotations}
...
main() {
    clear
    print_banner
    echo "Starting NetworkMapper v2 local environment setup..."
    check_os
    check_python
    install_system_deps
    setup_venv
    install_python_deps
    create_directories
    # This script no longer prints the final instructions, as the Makefile's install target will.
}
