...
def test_enrichment_merge():
    ...
    original.update({
        ...
        "services": enriched_data.get("services", []) if enriched_data.get("services") else original.get("services", []),
...
async def test_full_enrichment():
    ...
    print("\nAfter enrichment:")
    for d in enriched:
        print(f"  {d['ip']}: services={d.get('services', [])}, vendor={d.get('vendor', '')}, os={d.get('os', '')}")
