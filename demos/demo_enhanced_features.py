...
def demo_scan_metadata():
    ...
    console.print(
        f"[cyan]• Vulnerabilities:[/cyan] {interface._count_vulnerabilities(sample_devices)}"
    )
    console.print(f"[cyan]• Target Range:[/cyan] {config['target']}")
    console.print(
        f"[cyan]• Discovered Subnets:[/cyan] {', '.join(interface._get_discovered_subnets(sample_devices))}"
    )
