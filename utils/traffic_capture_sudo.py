...
def capture_packets(interface, duration, output_file):
    ...
    debug_info = {"interface": interface, "duration": duration, "start_time": start_time}

    def packet_handler(pkt):
        # Don't check duration here - let stop_filter handle it
        packet_info = {"timestamp": time.time(), "size": len(pkt)}
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print(
            json.dumps(
                {"error": "Usage: traffic_capture_sudo.py <interface> <duration> <output_file>"}
            )
        )
        sys.exit(1)
