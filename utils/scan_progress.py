...
class ScanProgressIndicator:
    ...
    def start(self, scanner, target: str, scan_type: str):
        ...
        self._task_id = self._progress.add_task(
            "Scanning...", total=100, status="Initializing scan..."
        )
    ...
    def _update_progress_loop(self):
        ...
        while not self._stop_event.is_set():
            try:
                if (
                    self._scanner
                    and hasattr(self._scanner, "hosts_completed")
                    and hasattr(self._scanner, "total_hosts")
                ):
                    ...
                    self._progress.update(
                        self._task_id, completed=min(percentage, 100), status=status
                    )
    ...
    def stop(self):
        ...
        if self._progress and self._task_id is not None:
            # Show completion
            self._progress.update(
                self._task_id, completed=100, status="[bold green]Scan complete![/bold green]"
            )
...
class MinimalScanIndicator:
    ...
    def start(self, target: str, scan_type: str):
        ...
        self._status = self.console.status(
            f"[bold cyan]Scanning {target}[/bold cyan] ({scan_type} scan)...", spinner="dots"
        )
        self._status.start()
