...
class SNMPManager:
    ...
    def enrich_devices(self, devices: List[Dict], max_workers: int = 10) -> List[Dict]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_device = {
                executor.submit(self.enrich_device, device.copy()): device for device in devices
            }
            ...
            for oid, value in if_table.items():
                ...
                if len(parts) >= 2:
                    ...
                    if if_property.endswith(".2.2.1.2"):  # ifDescr
                        ...
                        interface_data[if_index]["type"] = self.INTERFACE_TYPES.get(
                            if_type, f"unknown({if_type})"
                        )
                        ...
                        interface_data[if_index]["admin_status"] = (
                            "up" if int(value) == 1 else "down"
                        )
                    elif if_property.endswith(".2.2.1.8"):  # ifOperStatus
                        interface_data[if_index]["oper_status"] = (
                            "up" if int(value) == 1 else "down"
                        )
        ...
        for oid, value in processor_table.items():
            if oid.endswith(".3.3.1.2"):  # hrProcessorLoad
                ...
                if len(parts) > 0:
                    ...
                    processors.append({"index": int(cpu_index), "load_percent": load})
