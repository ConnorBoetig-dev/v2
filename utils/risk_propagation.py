...
class RiskPropagationAnalyzer:
        ...
        for device in devices:
            ...
            if "dependencies" in device:
                for dep_hostname in device["dependencies"]:
                    ...
                    dep_device = next((d for d in devices if d.get("hostname") == dep_hostname), None)
                    if dep_device:
    ...
    def _infer_dependencies(self, device: Dict, all_devices: List[Dict]):
        ...
        if device_type not in ["router", "firewall"]:
            gateways = [d for d in all_devices if d.get("type") in ["router", "firewall"] or 
                       d.get("is_gateway", False)]
        ...
        if device_type != "dns_server":
            dns_servers = [d for d in all_devices if d.get("type") == "dns_server" or
                          any("dns" in str(s).lower() for s in d.get("services", []))]
        ...
        if device.get("os", "").lower().startswith("windows") and device_type != "domain_controller":
            dcs = [d for d in all_devices if d.get("type") == "domain_controller" or
                   any("ldap" in str(s).lower() for s in d.get("services", []))]
            for dc in dcs:
    ...
    def simulate_device_failure(self, failed_device_ip: str) -> ImpactAnalysis:
        ...
        analysis.impact_severity = self._calculate_severity(
            ...
            analysis.critical_services_lost
        )
        ...
        analysis.mitigation_suggestions = self._generate_mitigations(
            failed_device, 
            analysis.critical_services_lost,
            analysis.redundancy_available
        )
        
        return analysis
    
    def _calculate_severity(self, failed_device: Dict, impact_count: int, 
                          services_lost: List[str]) -> str:
    ...
    def _check_redundancy(self, failed_device: Dict, failed_services: List[str]) -> bool:
        ...
        similar_devices = [
            d for d in self.device_map.values() 
    ...
    def _generate_mitigations(self, failed_device: Dict, services_lost: List[str], 
                            has_redundancy: bool) -> List[str]:
    ...
    def find_single_points_of_failure(self) -> List[Tuple[str, int]]:
        ...
        for device_ip, device in self.device_map.items():
            ...
            if (impact.total_impacted > 10 or 
                impact.impact_severity in ["High", "Critical"] or
                len(impact.critical_services_lost) > 0):
        ...
        for service_type, service_names in self.critical_services.items():
            ...
            if len(providers) < 2:
                report["redundancy_gaps"].append({
                    ...
                    "providers": [{"ip": p["ip"], "hostname": p.get("hostname", "N/A")} 
                                 for p in providers]
                })
