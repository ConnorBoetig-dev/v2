...
class SNMPConfig:
    ...
    def interactive_setup(self, force_prompt: bool = False) -> Tuple[bool, Dict]:
        ...
        self.console.print(
            "[dim]Provides hostname, uptime, interface counts, and system details[/dim]"
        )
        ...
        if not config:
            self.console.print(
                "[yellow]SNMP configuration cancelled - proceeding without enrichment[/yellow]"
            )
            return False, {}
    ...
    def _get_snmpv3_config(self) -> Optional[Dict]:
        ...
        while True:
            auth_password = Prompt.ask("[bold]Authentication Password[/bold]", password=True)
        ...
        use_privacy = Confirm.ask("[bold]Enable encryption (privacy)?[/bold]", default=True)
        ...
        if use_privacy:
            while True:
                ...
                if not priv_password:
                    ...
                    self.console.print(
                        "[red]Encryption password must be at least 8 characters[/red]"
                    )
        else:
    ...
    def _display_existing_config(self, config: Dict) -> None:
        ...
        if config.get("version") in ["v1", "v2c"]:
            ...
            masked = (
                community[0] + "*" * (len(community) - 2) + community[-1]
                if len(community) > 2
                else "*" * len(community)
            )
