...
class ExportManager:
    ...
    def export_to_pdf(self, devices: List[Dict], 
        ...
        if changes and any(changes.get(k, []) for k in ['new_devices', 'missing_devices', 'changed_devices']):
            elements.append(Paragraph("Network Changes", heading_style))
            
            if changes.get('new_devices'):
                elements.append(Paragraph(f"New Devices ({len(changes['new_devices'])})", styles['Heading3']))
                ...
                new_table = Table(new_data, colWidths=[1.5*inch, 2*inch, 1.5*inch, 1.5*inch])
            ...
            if changes.get('missing_devices'):
                elements.append(Paragraph(f"Missing Devices ({len(changes['missing_devices'])})", styles['Heading3']))
        ...
        if critical_devices:
            ...
            critical_table = Table(critical_data, colWidths=[1.2*inch, 1.5*inch, 1.2*inch, 1.8*inch, 1.8*inch])
        ...
        elements.append(Paragraph("Showing first 50 devices. See Excel export for complete list.", styles['Italic']))
        ...
        device_table = Table(device_data, colWidths=[1*inch, 1.5*inch, 1.3*inch, 1*inch, 1.5*inch, 1.2*inch])
    ...
    def export_to_excel(self, devices: List[Dict], 
                       changes: Optional[Dict] = None,
                       filename: Optional[str] = None) -> Path:
        ...
        summary_sheet.append(["Critical Devices:", len([d for d in devices if d.get('critical', False)])])
        ...
        headers = [
            ...
            "Device Type", "Operating System", "Critical", 
            "Open Ports", "Services", "Notes", "Last Seen"
        ]
        ...
        if changes and any(changes.get(k, []) for k in ['new_devices', 'missing_devices', 'changed_devices']):
            ...
            # Missing devices
            if changes.get('missing_devices'):
                changes_sheet.append([f"MISSING DEVICES ({len(changes['missing_devices'])})", "", "", ""])
                changes_sheet.merge_cells(f"A{row_num}:D{row_num}")
        ...
        subnet_sheet.append(["Subnet", "Device Count", "Router", "Switches", "Servers", "Workstations", "Other"])
    ...
    def export_to_json(self, devices: List[Dict], 
                      changes: Optional[Dict] = None,
                      filename: Optional[str] = None) -> Path:
    ...
    def export_to_csv_enhanced(self, devices: List[Dict], 
                              filename: Optional[str] = None) -> Path:
        ...
        if 'open_ports' in df.columns:
            df['open_ports'] = df['open_ports'].apply(lambda x: ';'.join(map(str, x)) if isinstance(x, list) else '')
