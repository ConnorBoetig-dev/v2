...
class DeviceAnnotator:
    ...
    def annotate_device(self, device: Dict):
        ...
        annotation = self.annotations.get(
            ip, {"critical": False, "notes": "", "tags": [], "location": "", "owner": ""}
        )

        # Critical flag
        annotation["critical"] = Confirm.ask(
            "Mark as critical infrastructure?", default=annotation["critical"]
        )
        ...
        annotation["location"] = Prompt.ask(
            "Physical location", default=annotation.get("location", "")
        )

        # Owner
        annotation["owner"] = Prompt.ask("Owner/Department", default=annotation.get("owner", ""))
    ...
    def bulk_annotate(self, devices: List[Dict]):
        ...
        selections = Prompt.ask("Enter device numbers to annotate (comma-separated)")
