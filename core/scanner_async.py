...
class AsyncNetworkScanner:
    ...
    async def scan(
        ...
        if len(subnets) > 1:
            self.console.print(
                f"[cyan]🔀 Splitting scan into {len(subnets)} parallel subnet scans[/cyan]"
            )
    ...
    async def _scan_subnet_discovery(
        self, subnet: str, use_masscan: bool, needs_root: bool
    ) -> List[Dict]:
    ...
    async def _scan_subnet_detailed(
        self, subnet: str, scan_type: str, needs_root: bool
    ) -> List[Dict]:
        ...
        try:
            ...
            with Progress(
                ...
                try:
                    ...
                    self.console.print(
                        "[yellow]Please install nmap: sudo apt install nmap[/yellow]"
                    )
                    return chunk
        ...
        with Progress(
            ...
            for i in range(0, len(devices), chunk_size):
                ...
                try:
                    ...
                    progress.update(task, status=f"Chunk {chunk_num} failed - continuing...")
    ...
    async def _enrich_deeper_chunk_async(
        self, chunk: List[Dict], chunk_idx: int, total_chunks: int
    ) -> List[Dict]:
        ...
        async with self._enrich_semaphore:
            ...
            self.console.print(
                f"[dim]Deep scanning chunk {chunk_idx + 1}/{total_chunks} ({len(ips)} IPs)[/dim]"
            )
                ...
                if proc.returncode == 0:
                    ...
                    for e in enriched:
                        if e["ip"] in device_map:
                            ...
                            e["open_ports"] = list(
                                set(original.get("open_ports", []) + e.get("open_ports", []))
                            )
                    ...
                    logger.warning(
                        f"Deeper enrichment failed for chunk {chunk_idx + 1}: {stderr.decode()}"
                    )
                    return chunk
        ...
        async with self._enrich_semaphore:
            ...
            try:
                cmd = [
                    ...
                    temp_file,
                    "--max-rtt-timeout",
                    "200ms",  # Increased for reliability
                    "--max-retries",
                    "2",  # More retries for better detection
                ] + ips
                ...
                try:
                    ...
                    self.console.print(
                        f"[yellow]⚠ Enrichment timeout for chunk - trying without OS detection[/yellow]"
                    )
                    ...
                    process = await asyncio.create_subprocess_exec(
                        *cmd_no_os, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
                    )
    ...
    async def _query_snmp_device_async(self, device: Dict, snmp_manager: Any) -> Dict:
        ...
        async with self._snmp_semaphore:
            try:
                ...
                enriched = await loop.run_in_executor(None, snmp_manager.enrich_device, device)
                return enriched
            except Exception as e:
    ...
    async def _ensure_sudo_access_async(self) -> bool:
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                "sudo", "-n", "true", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            await process.wait()
